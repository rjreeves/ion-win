...
pub(crate) fn handle_key_event(
    ...
    match windows_key_event {
        WindowsKeyEvent::KeyEvent(key_event) => {
            ...
            // next surrogate value. Windows Terminal may interleave the release for the high
            // surrogate before sending the low surrogate during paste, so releases must not
            // break a pending pair.
        ...
        WindowsKeyEvent::Surrogate(new_surrogate) => {
            // ReadConsoleInputW reports a press and a release for each UTF-16 code unit.
            // Pair only the presses; otherwise high-release is incorrectly paired with
            // high-press and both halves of every non-BMP character are discarded.
...
mod tests {
    ...
    fn surrogate(key_down: bool, u_char: u16) -> KeyEventRecord {
        KeyEventRecord {
            key_down,
            repeat_count: 1,
            virtual_key_code: 0,
            virtual_scan_code: 0,
            u_char,
            // All-zero is the documented "no modifiers" bit pattern.
            control_key_state: unsafe { std::mem::zeroed() },
        }
    }

    #[test]
    fn surrogate_release_between_halves_does_not_break_pair() {
        let mut buffer = None;

        assert_eq!(handle_key_event(surrogate(true, 0xD83D), &mut buffer), None);
        assert_eq!(
            handle_key_event(surrogate(false, 0xD83D), &mut buffer),
            None
        );
        assert_eq!(
            handle_key_event(surrogate(true, 0xDC69), &mut buffer),
            Some(Event::Key(KeyEvent::new(
                KeyCode::Char('\u{1F469}'),
                KeyModifiers::NONE
            )))
        );
    }
