...
## Module map (7,892 lines across 20 files)

| File | Lines | Purpose |
|---|---|---|
| `interp.rs` | 1846 | The core: tokenizer, `$`/`@` expansion, variable scope stack, `let`/`export`/`drop`, method-call and process-expansion dispatch, `echo` output/capture. The biggest and most load-bearing file. |
| `shell.rs` | 1520 | REPL loop and prompt rendering (`PROMPT` function support), block/control-flow execution (`if`/`while`/`for`/`fn`/`match`/`case`), `cd`, `&&`/`||`/`and`/`or` chaining, dispatch of all builtins |
...
| `pipeline_exec.rs` | 370 | Real OS pipeline execution (`Command`/`Stdio` chaining), background-job registration |
