...
**Deliberately not built**: no sigil/interpolation support for table variables (`$mytable`/`${mytable}` in an ordinary string context still errors "variable does not exist" — the only supported way to reference one is as a bare pipeline-stage name); `exists`/`intersects` don't know about table variables either. Both are reasonable follow-ups, not required for this slice.
