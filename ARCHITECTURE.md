...
1. **A foreground external execution** (single command, pipeline stage, or captured process expansion) — children launched through the normal shell, pipeline, and `$()`/`@()` capture paths are spawned via `jobctl::new_command`, which sets the Windows-specific `CREATE_NEW_PROCESS_GROUP` creation flag. This isolates the child from the default console-wide Ctrl+C broadcast (which would otherwise also hit the shell itself and every other child it has spawned). The active `ExecutionManager` registry is now the sole source of foreground PIDs: `jobctl::request_interrupt` takes a short snapshot of processes belonging to running/cancelling foreground executions, releases the manager lock, and forwards a **`CTRL_BREAK_EVENT`** to each PID via `GenerateConsoleCtrlEvent`. Background and detached executions are excluded by execution mode. Captured expansions use redirected terminal/I/O modes but remain foreground executions, so they preserve synchronous capture while becoming interruptible and reportable.
...
**Status: command-producing execution ownership implemented.** `src/execution.rs` defines the immutable spec, invocation context, lifecycle types, sanitized history record, validated transitions, and bounded in-memory `ExecutionManager` registry. Ordinary foreground commands, foreground pipelines, tracked background `&` pipelines, and external `$()`/`@()` process expansions now create executions; the manager is the only runtime owner/waiter of their child handles. `jobctl` remains the Windows process-group/Ctrl+Break backend, using an execution-derived PID view. This section supersedes the old architectural idea of a general-purpose "Jobs Registry." `Job` is not an ion domain type: on Windows it is too easily confused with a kernel Job Object, a scheduled task, or the legacy shell `jobs` command.
...
The simplest valid external execution is still `CreateProcessW` (or `std::process::Command`) plus ordinary standard-handle wiring and wait. A Windows Job Object is optional at the abstraction boundary. Managed foreground and tracked-background launches now make a best-effort attachment to one private Job Object per `Execution`; a host that rejects nested assignment does not turn an already successful process spawn into a shell failure. Explicitly disowned `&!` launches remain outside management.

`src/job_object.rs` is the backend-private RAII boundary for creation, assignment, accounting queries, explicit tree termination, and handle closure. `ExecutionManager` alone owns these wrappers and removes them with terminal/disowned executions. Its current close policy preserves processes: dropping the wrapper never terminates a child or descendant, which is required for `disown` and ordinary process-spawn semantics. Because Windows cannot detach one assigned process, disowning any stage releases the whole pipeline's Job Object while retaining ordinary child-handle management for the other stages. Tree termination is an explicit call, ready for cancellation escalation policy rather than an accidental `Drop` side effect.
...
Cancellation is requested on the execution, never directly on an arbitrary `Process` from higher layers. This policy is now implemented: the Ctrl+C handler asks `ExecutionManager` to transition every running foreground execution to `Cancelling`, set its invocation cancellation token, and return a short-lived PID/Execution-ID snapshot. `jobctl` sends the established targeted Ctrl+Break outside the manager lock, then a detached 750 ms grace timer asks the manager to terminate the private Job Object only if that execution is still cancelling. Executions without an assigned Job Object retain cooperative Ctrl+Break behavior. All foreground wait paths finalize a cancelling execution as `Cancelled`, preserving any observed exit code but normalizing the reportable message to `cancelled`. Background and detached/disowned executions are excluded. Job Object kill-on-close remains disabled so dropping a Rust value cannot accidentally change user-visible lifetime.
...
The first persistent definition slice is implemented in `src/task.rs`, `src/schedule.rs`, and dedicated `tasks`/`schedules` tables in `src/state.rs`. `TaskDefinition` and `ScheduleDefinition` are immutable after construction and their versioned JSON representations contain no execution ID, process handle, lifecycle state, OS registration, or history. A task stores a validated name, external command/arguments, and absolute working directory. A schedule stores a validated name/task reference, enabled policy, and a platform-neutral once/daily/logon/startup trigger; once timestamps require an explicit RFC 3339 offset and daily triggers require a real IANA timezone. Both registries support atomic create-versus-replace, lookup, sorted listing, and deletion through the state worker and memory fallback. Schedule insertion requires its task to exist, and task deletion is rejected while any schedule references it.

`task create [--force] NAME [--] COMMAND [ARG...]`, `task list`, `task show NAME`, `task delete NAME`, and `task run NAME` are the task adapters. Creation captures the current directory. Running resolves relative commands against that captured directory, produces a foreground `ExecutionSpec`, and submits it through `ExecutionManager`; it never creates a parallel task-specific runtime path.

`schedule create [--force] [--disabled] NAME TASK TRIGGER...`, `schedule list/show/delete/enable/disable/run` are implemented with `src/task_scheduler.rs` as the Windows backend. Registrations use namespaced root tasks (`\ion-win-NAME`) and Task Scheduler XML for once, IANA-timezone-aware daily, logon, and startup triggers. The action launches ion-win with a backend-private, versioned task snapshot under `%APPDATA%\ion-win\scheduled`; this avoids the state database's intentional single-process writer lock while ensuring the scheduled process still reconstructs an `ExecutionSpec` and enters `ExecutionManager`. Create/replace/delete/enable/disable apply OS changes first and compensate or roll back if registry persistence fails. `schedule delete --force` is the explicit recovery path for a definition whose OS registration was removed externally. Replacing or deleting a task referenced by a schedule is rejected so snapshots cannot silently become stale.
...
4. **Complete:** add the private Windows `JobObject` wrapper and optional managed-launch policy. Tests cover creation/accounting, assignment, preserve-on-close cleanup, and explicit tree termination; guaranteed race-free containment may later upgrade launch to suspended-create/assign/resume without changing `Execution`.
5. **Complete:** route foreground cancellation through `ExecutionManager`, preserving Ctrl+Break as the cooperative first step and escalating assigned Job Objects after a grace period while recording `Cancelled` consistently.
6. **Complete:** add immutable persistent `TaskDefinition`/`ScheduleDefinition` records, task and schedule command suites, and the Windows Task Scheduler adapter. Manual and scheduled task launches both produce and submit an `ExecutionSpec`; OS registration and snapshots remain backend-private.
